...
of a function `main` never calls fails the build here. A function with a bare
parameter that nothing calls gets the [warnings](errors.md#warnings) `check`
prints, and the build still writes the artifact. A package a project depends on is compiled
